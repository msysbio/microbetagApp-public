...
from microbetag.scripts.utils import map_otu_to_ncbi_tax_level_and_id, otu_faprotax_functions_assignment, is_tab_separated, ensure_flashweave_format, edge_list_of_ncbi_ids, export_phen_traits_to_file
from microbetag.scripts.db_functions import check_connection_to_db, get_phenDB_traits, get_column_names
...
def main():
    ...
    if PHEN_DB:
        ...
        if not check_connection_to_db(USER_NAME, PASSWORD, HOST, DB_NAME):
            logging.error(
                "microbetag cannot link to the microbetag db. Check host server, user name and password credentials.")
            sys.exit(0)
            ...
            if not check:
                if gtdb_id[:3] == "GCA":
                    ...
                    try:
                        ...
                        pass
            if check:
                tr = otu_taxid_level_repr_genome.loc[otu_taxid_level_repr_genome["gtdb_gen_repr"] == gtdb_id, [
                    "Species", "ncbi_tax_id"]]
                sp = tr.iloc[0, 0]
    ...
    if PATHWAY_COMPLEMENTARITY:
        ...
        set_of_ncbi_ids_with_available_genomes = set(
            os.listdir("ref-dbs/kegg_genomes/"))
        ...
        for pair in edgelist_to_pc:
            ...
            if pair["ncbi_tax_level_node_a"] == pair["ncbi_tax_level_node_b"] == "mspecies":
                ...
                print(
                    pair["gtdb_gen_repr_node_a"],
                    pair["gtdb_gen_repr_node_b"])
