...
def add_ncbi_attributes(nodeAttributes, node_counter, case, seq):
...
def check_env_edge_node(pot_nodes, set_of_nodes, node_counter, seq_to_nodeID, seq_map, nodes, nodeAttributes):
    """
    pot_nodes: list
    set_of_nodes:
    ...
    for pot_node in pot_nodes:
        if pot_node not in set_of_nodes:
            node_counter += 1
            node = {"@id": node_counter, "n": pot_node}
            nodes["nodes"].append(node)
            seq_to_nodeID[pot_node] = node_counter
            # [NOTE] there is a chance where a taxon has an edge only with metavars
            if pot_node in seq_map["seqId"].to_list():
                case = seq_map[seq_map["seqId"] == pot_node]
                nodeAttributes = add_ncbi_attributes(nodeAttributes, node_counter, case, pot_node)
            else:
                nodeAttributes["nodeAttributes"].append({"po": node_counter, "n": "@id", "v": pot_node, "d": "string"})
                nodeAttributes["nodeAttributes"].append({"po": node_counter, "n": "shared name", "v": pot_node, "d": "string"})
                nodeAttributes["nodeAttributes"].append({"po": node_counter, "n": "display name", "v": pot_node, "d": "string"})
                nodeAttributes["nodeAttributes"].append({"po": node_counter, "n": "microbetag::ncbi-tax-level", "v": "metavar", "d": "string"})
    return nodes, node_counter, seq_to_nodeID, nodeAttributes


def build_cx_annotated_graph(edgelist_as_df, edgelist_as_a_list_of_dicts, seq_map, env_edges, cfg, out_dir, children_df=None):
    """
    ...
    for seq in set_of_nodes:
        ...
        # NCBI attribute
        nodeAttributes = add_ncbi_attributes(nodeAttributes, node_counter, case, seq)
    ...
    if len(env_edges) > 0:
        ...
        # consider manta as well
        for env_edge in env_edges:
            source = env_edge["node_a"]; target = env_edge["node_b"]
            nodes, node_counter, seq_to_nodeID, nodeAttributes = check_env_edge_node([source, target],
                                                                                     set_of_nodes,
                                                                                     node_counter,
                                                                                     seq_to_nodeID,
                                                                                     seq_map,
    ...
    if len(env_edges) > 0:
        # [{'node_a': 'ENV4_A', 'node_b': 'ENV4_C', 'score': -0.774596631526947}, .. ]
        for env_edge in env_edges:
            id_a, id_b = seq_to_nodeID[env_edge["node_a"]], seq_to_nodeID[env_edge["node_b"]]
            edge = {"@id": edge_counter, "s": id_a, "t": id_b, "i": "metadata driven"}
            edges["edges"].append(edge)

            edgeAttributes["edgeAttributes"].append({"po": edge_counter, "n": "microbetag::weight", "v": str(env_edge["score"]), "d": "double"})
            edgeAttributes["edgeAttributes"].append({"po": edge_counter, "n": "interaction type", "v": "metadata driven", "d": "string"})
            edgeAttributes["edgeAttributes"].append({"po": edge_counter, "n": "shared name", "v": " ".join([env_edge["node_a"], "(metadata bind with)", env_edge["node_b"]]), "d": "string"})
            edge_counter += 1
