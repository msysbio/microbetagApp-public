...
# Yeah, it's arbitrary
def calculate_seeds(graph, onlyGiant=None,
                    minComponentSize=None, seedThreshold=0):
    ...
    if onlyGiant or minComponentSize:
        ...
        nComponents, nodeToComponent = depth_first_search(
            graph, nodeOrder, reverse_graph)[1:3]
    ...
    for group in SeedGroups.keys():
        if len(SeedGroups[group]) < sccSize[group]:
            if len(SeedGroups[group]) == 0 and 1 / \
                    float(sccSize[group]) <= seedThreshold:
                print("Removing Seed Group", group, "of size", sccSize[group])
                del SeedGroups[group]
            else:
                print("Serious problem in SeedGroup", group, ": Expected",
                      sccSize[group], "and found", len(SeedGroups[group]), ".")
